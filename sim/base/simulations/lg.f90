...
integer :: i,j,t,timePrint=1,tt,dCount=0,un=4,seedSize,istat=0,R,deltaR
    ...
    function hamXY(i,j,theta,kappa,mu)
        ...
        do i=1,N
            do j=1,N
                ...
                if ( (sqrt(real((ii-N/2)**2 + (jj-N/2)**2)) .lt. R) .and. &
                    (sqrt(real((ii-N/2)**2+(jj-N/2)**2)) .gt. (R-deltaR))) then
                    muTemp = muVector(t)
