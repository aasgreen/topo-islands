...
integer :: i,j,t,timeprint=1,tt,dcount=0,un=4,seedsize,istat=0,x_delt, x_pos
...
do t=1,int(endt)
    ...
    if (1 .eq. 1) then !this is not log time, so we need all time steps
       write(66,*) x_pos, 1.0*real(t)/endt
       write(67,*) filenames
       !print *, trim(filenames)
    ...
    subroutine update(state,newstate,hgrid,lnoise,n)
        ...
        do i=1,n
            do j=1,n
                ...
                if  ( ( ii .lt. (x_pos + x_delt)) .and. &
                    (ii .gt. (x_pos-x_delt)) ) then
