...
!This program will create a line where the spin is forced to align perpendicular to the rest of the field. Then, we will
!expand this line out from the center of the grid, creating a pressure wave. 
...
real(8) :: x,kappa,windingn, lnoise, x_delt
...
integer :: i,j,t,timeprint=1,tt,dcount=0,un=4,seedsize,istat=0, x_pos
...
do t=1,int(endt)
    ...
    grid=gridplusdelta
    if (( t .gt. 10 )) then
        call line_boundary(x_pos,x_delt,t)
    endif
    ...
    if (1 .eq. 1) then !this is not log time, so we need all time steps
       write(66,*) x_pos, x_delt, 1.0*real(t)/endt
       write(67,*) filenames
       !print *, trim(filenames)
...
contains
    ...
    subroutine line_boundary(x_pos,x_delt,t)
        !calculate the radius of the island at a given time
        integer :: t
        real(8) :: x_delt
        integer :: x_pos
        ! case 2: grow, hit 50 and then shrink
        if (1.0*t/endt .le. .5) then
            x_pos = int(n/2) !this will put the line in the middle of the simulation

            x_delt = x_delt +.005 !5 px/s. !this should expand out
           ! x_delt = 10
        else if ( (1.0*t/endt .gt. .8)) then
            x_pos = -499
            x_delt = 0d0
            
        end if
    end subroutine line_boundary
    ...
    subroutine update(state,newstate,hgrid,lnoise,n)
        ...
        do i=1,n
            do j=1,n
                ...
                if  ( ( real(ii) .lt. (real(x_pos) + x_delt)) .and. &
                    (real(ii) .gt. (real(x_pos)-x_delt)) ) then
