...
#            dfN.sort_values(by=['time'],inplace=True)
            ...
            elif fname == 'defectGrid.h5':
                print('h5 file found'+fname)
                dhf = h5py.File(args.fName+'/'+fname)
                dfN = pd.DataFrame(list(dhf.keys()))
                print(len(list(dhf.keys())))
#            dfN = pd.DataFrame(glob.glob(args.fName+'/defect*.dat'))
                params = np.loadtxt('params.txt')
#            print(fN)
                print(dfN.head())
...
#            dfN.sort_values(by=['time'],inplace=True)
            #Sort fileNames by number
                ...
                dimSeq = [np.array(dhf.get(f)) for f in dfN[0]]
...
def binMatrix(arr, new_shape):
    ...
    return arr.reshape(shape).mean(-1).mean(1)j
def binMatrix2(grid,step):
    x,y = grid.shape
...
if __name__ == '__main__':
    ...
    imSeq, fN, dimSeq, dfN = read_files(args)
    imSeq = imSeq[::len(imSeq)//args.nFrame]
    fN = fN[::len(fN)//args.nFrame]
    dimSeq = dimSeq[::len(dimSeq)//args.nFrame]
    dfN = dfN[::len(dfN)//args.nFrame]
...
#    dimSeq = [np.loadtxt(f) for f in dfN[0]]
    ...
    #pDefect = np.vstack([xpDefect,ypDefect])
    #mDefect = np.vstack([xmDefect,ymDefect])
    ...
    progpDefect = np.vstack([pxpDefect,pypDefect])
    progmDefect = np.vstack([pxmDefect,pymDefect])

    #label defects (setup to label color for python tagging, p/m for fortran tags
    #[dCirc[tuple(loc)].set_facecolor('blue') for loc in mDefect]
    #[dCirc[tuple(loc)].set_facecolor('red') for loc in pDefect]


    [dText[tuple(loc)].set_text('+') for loc in progpDefect if progpDefect]
    [dText[tuple(loc)].set_text('-') for loc in progmDefect if progmDefect]
    ...
    imd=ax2.imshow((fMain%(2*np.pi))*180/np.pi,cmap='twilight',origin='top')
    plt.colorbar(imd,ax=ax2, orientation='horizontal')
    ...
    def update_quiver(num,Q,X,Y):
        ...
        #i = int(t*fps_in)
        print( "{}% finished".format(num/len(imSeq)))
        frame = imSeq[get_frame(num)]
        fMain = frame
        frame = binMatrix(frame,[20,20])
...
#
    ...
    anim = animation.FuncAnimation(fig,update_quiver,fargs=(Q,X,Y),interval=201,blit=False, frames=200)


    anim.save('b-{}monte-carlo-xy-texture.avi'.format(beta), writer='ffmpeg', extra_args=["-threads", "4"])
