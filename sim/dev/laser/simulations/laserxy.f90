...
!This program will create a 'laser' bubble in the center of the grid,
!with a given temperature that is different from the background, this is
!to demonstrate how changing the local energy of a system leads to defect pairs being created
character(100) :: buffer
integer :: n = 200, endt=1001,seedn
real(8) :: laser_beta, beta,mu,measuredt,zeroe, meank,avedelphi
real(8), allocatable, dimension(:) :: muvector
integer, dimension(100) :: tpoints
integer, allocatable, dimension(:) :: logtpoints
real(8), allocatable, dimension(:,:) :: grid,dgrid,gridplusdelta, hgrid
real(8) :: x,kappa,windingn,laser_noise, lnoise
...
read(buffer,*) laser_beta !temperature of the laser
...
laser_noise = sqrt(12.0*1.0/laser_beta)
...
!the following code deals with the radius of an island. We are going to reuse this idea for our laser.
...
contains
    ...
    subroutine update(state,newstate,hgrid,lnoise_background,n)
        real(8) :: theta, thetap,x,dele,e1,e2,lnoise_background,fluce,u,force,fieldphi,mutemp, lnoise
        integer :: n,ii,jj,i,j
        ...
        do i=1,n
            do j=1,n
                ...
                call random_number(x)
                jj = 1+floor((n)*x)
                !see if we are within the circle
                if  (sqrt(real((ii-n/2)**2 + (jj-n/2)**2)) .le. islandr) then
                    lnoise = laser_noise !using global variable laser_noise
                else
                    lnoise = lnoise_background
                endif
               ! lnoise = lnoise_background
                mutemp = mu !this is a leftover from when this code was dealing with islands, and we were fucking with mu
                fieldphi = 0.
                mutemp = 0.
                theta = grid(ii,jj)
                ...
                hgrid(ii,jj) = hamxy(ii, jj, newstate(ii,jj), kappa, mu)+4*kappa !this will normalize so zero is actually the
        !energy
